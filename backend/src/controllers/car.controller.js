...
export const findAllCarsByFilters = async (req, res) => {
    ...
    try {
        ...
        if(req.body.brands.length) filters['brand'] = { "$in" : req.body.brands } 

        if(req.body.years.length) filters['year'] = { "$in" : req.body.years }
        
        if(req.body.carClasses.length) filters['carClass'] = { "$in" : req.body.carClasses }
